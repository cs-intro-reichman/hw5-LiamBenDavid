public class Wordle {
    ...
    public static void computeFeedback(String secret, String guess, char[] resultRow) {
        ...
        for (int guessCharIndex = 0; guessCharIndex < guess.length(); guessCharIndex++){
            char status = '_';
            boolean existing = containsChar(secret, guess.charAt(guessCharIndex));

            if (existing) {
                if (secret.charAt(guessCharIndex) == guess.charAt(guessCharIndex)) {
                    status = 'G';
                } else {
                    status = 'Y';
                }
            }
            resultRow[guessCharIndex] = status;
    ...
    // Printsrow the game board up to ; (inclusive).
